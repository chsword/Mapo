...
Dock.init = function () {
    ...
    $("#dock_page_size").button({
        onMousedown: function () {
            ...
            var item = $("#page_size_list")
                .children("li[w=" + Model.define.page.width + "][h=" + Model.define.page.height + "]");
            if (item.length > 0) {
    ...
    //是否显示网格
    $("#dock_page_showgrid").bind("change",
        function () {
...
Dock.showView = function (name) {
     ...
     * @param {} fillStyle
     */
    Dock.setFillStyle = function (fillStyle) {
        ...
        if (fillStyle.type == "solid") {
            ...
            //渐变类型
            $("#gradient_type").button()
                .setText($("#gradient_type_list").children("li[ty=" + fillStyle.gradientType + "]").text());
            $(".gradient_details").hide();
    ...
    Dock.update = function (drawNav) {
        if (this.currentView == "navigator") {
            ...
            if (count == 0) {
                ...
                var lineStyleCls = $("#line_style_list").children("li[line=" + shape.lineStyle.lineStyle + "]")
                    .children().attr("class");
            ...
            if (shapeCount == 0) {
                ...
                $("#dock_metric_angle").button().enable().spinner()
                    .setValue(Math.round(shape.props.angle / Math.PI * 180) + "°");
            }
...
Dock.loadHistorys = function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.versions.length == 0) {
                $("#history_container")
                    .html('<div style="padding: 20px 10px;">暂时没有历史版本。<br/>每次修改，都会为您保存一个新的历史版本。</div>');
            } else {
                ...
                for (var i = 0; i < data.versions.length; i++) {
                    ...
                    var newVersion =
                        $('<li vid="' +
                            ...
                            v.updateTime +
                            '</div><div class="version_name"></div></li>').appendTo($("#history_versions"));
                    ...
                    var remarkContainer =
                        $(
                            "<div class='history_remark'><div class='remark_container'><div class='remark_text'></div><a onclick='Dock.editHistoryRemark(event, \"" +
                            v.versionId +
                            "\")' href='javascript:'>注释</a></div></div>").appendTo(newVersion);
                    if (v.remark) {
                        remarkContainer.find(".remark_text").text(v.remark);
                    }
                    remarkContainer.append(
                        "<div class='edit_container'><textarea class='input_text' onclick='event.stopPropagation()'></textarea><a href='javascript:' class='save'>保存</a>&nbsp;&nbsp;<a href='javascript:' class='cancel'>取消</a></div>")
                }
                Dock.resetVersions();
            }
        }
    ...
    Dock.editHistoryRemark = function (event, versionId) {
        ...
        editContainer.children(".save").bind("click",
            function (e) {
                e.stopPropagation();
        ...
        editContainer.children(".cancel").bind("click",
            function (e) {
                e.stopPropagation();
     ...
     * 取消版本注释的编辑
     */
    Dock.cancelHistoryRemark = function () {
