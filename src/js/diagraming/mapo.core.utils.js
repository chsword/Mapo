 ...
 * 获取某一位置下的形状容器
 */
Utils.getShapeByPosition = function (x, y, findLinkpoint) {
    var focusShapes = [];
 ...
 * @return {}
 */
Utils.checkCross = function (p1, p2, p3, p4) {
    var flag = false;
 ...
 * @param {} rect2
 */
Utils.rectCross = function (rect1, rect2) {
    var minX1 = rect1.x;
 ...
 * @return {Boolean}
 */
Utils.rectInRect = function (rect, containerRect) {
 ...
 * 一个点是否在一个矩形中
 */
Utils.pointInRect = function (px, py, rect) {
 ...
 */
Utils.pointInLinker = function (point, linker, radius) {
 ...
 * @param {} linker
 */
Utils.getLinkerLength = function (linker) {
    var points = this.getLinkerLinePoints(linker);
    var len = 0;
    for (var pi = 1; pi < points.length; pi++) {
        var p1 = points[pi - 1];
 ...
 * 获取一个坐标范围内的形状
 */
Utils.getShapesByRange = function (range) {
    var result = [];
 ...
 * 创建图形的轮廓
 */
Utils.getControlBox = function (shapeIds) {
    var pos = {
        x1: null,
        y1: null,
        x2: null,
 ...
 * 获取图形的轮廓
 */
Utils.getShapesBounding = function (shapes) {
    var pos = {
        x1: null,
        y1: null,
        x2: null,
 ...
 * @param {} shapeId
 */
Utils.getShapeContext = function (shapeId) {
 ...
 */
Utils.selectShape = function (shapeIds, withCallback) {
    //如果是字符串，则为选择一个
 ...
 * 获取选中的连接线
 */
Utils.getSelectedLinkers = function () {
    var result = [];
    for (var i = 0; i < this.selectIds.length; i++) {
        var shapeId = this.selectIds[i];
        if (!Utils.isLocked(shapeId)) {
 ...
 * 获取选中的连接线的id
 */
Utils.getSelectedLinkerIds = function () {
    var result = [];
    for (var i = 0; i < this.selectIds.length; i++) {
        var shapeId = this.selectIds[i];
        if (!Utils.isLocked(shapeId)) {
 ...
 * 获取选中的形状的id
 */
Utils.getSelectedShapeIds = function () {
    var result = [];
    for (var i = 0; i < this.selectIds.length; i++) {
        var shapeId = this.selectIds[i];
        if (!Utils.isLocked(shapeId)) {
 ...
 * 获取选中的并且被锁定的的id集合
 */
Utils.getSelectedLockedIds = function () {
    var result = [];
    for (var i = 0; i < this.selectIds.length; i++) {
        var shapeId = this.selectIds[i];
        if (Utils.isLocked(shapeId)) {
 ...
 * @return {}
 */
Utils.getSelectedGroups = function () {
    var result = [];
    for (var i = 0; i < this.selectIds.length; i++) {
        var shapeId = this.selectIds[i];
        var shape = Model.getShapeById(shapeId);
 ...
 * @return {}
 */
Utils.isSelected = function (shapeId) {
 ...
 * @return {Boolean}
 */
Utils.isLocked = function (shapeId) {
 ...
 * @type {}
 */
Utils.linkerCursorTimer = null;

/**
 * 显示从一个形状上连出的连接线上的游标
 */
Utils.showLinkerCursor = function () {
    this.hideLinkerCursor();
    var ids = Utils.getSelectedIds();
    if (ids.length == 1) {
 ...
 * 移动连接线上的游标
 */
Utils.playLinkerCursor = function (cursors) {
 ...
 * 隐藏连接线游标
 */
Utils.hideLinkerCursor = function () {
 ...
 * 绘制连接线上的控件
 */
Utils.showLinkerControls = function () {
    this.hideLinkerControls();
    var ids = Utils.getSelectedIds();
    var linker = null;
    if (ids.length == 1) {
 ...
 * 隐藏连接线上的控件
 */
Utils.hideLinkerControls = function () {
 ...
 * @param shape 形状对象
 */
Utils.showAnchors = function (shape) {
 ...
 * @param shape 形状对象
 */
Utils.showLockers = function (shape) {
 ...
 * @return {}
 */
Utils.measureDistance = function (p1, p2) {
    var h = p2.y - p1.y;
    var w = p2.x - p1.x;
 ...
 * @return {}
 */
Utils.addToArray = function (array, element) {
    var index = array.indexOf(element);
    if (index < 0) {
        array.push(element);
    }
 ...
 * @param {} arr2
 */
Utils.mergeArray = function (arr1, arr2) {
 ...
 * @param {} y
 */
Utils.getCirclePoints = function (x, y, r) {
 ...
 * 获取连接点相对于形状的角度
 */
Utils.getPointAngle = function (shapeId, x, y, r) {
 ...
 * 获取角度的方向，分为上1右2下3左4
 */
Utils.getAngleDir = function (angle) {
    var pi = Math.PI;
 ...
 * 获取连接线上的几个控制点点
 */
Utils.getLinkerPoints = function (linker) {
    var points = [];
    if (linker.linkerType == "broken") {
        var pi = Math.PI;
 ...
 * @return {}
 */
Utils.getLinkerLinePoints = function (linker) {
    var points = [];
 ...
 * @param {} linker
 */
Utils.getLinkerBox = function (linker) {
    var points = this.getLinkerLinePoints(linker);
    var minX = points[0].x;
    var minY = points[0].y;
    var maxX = points[0].x;
 ...
 * 获取形状的边界
 */
Utils.getShapeBox = function (shape) {
 ...
 */
Utils.getRotatedBox = function (pos, angle, center) {
    if (angle == 0) {
        return pos;
    } else {
        //没有设置中心点，取形状的中心点
 ...
 * @param {} angle 旋转的角度
 */
Utils.getRotated = function (center, point, angle) {
    //先得到两点的距离，即圆形运动的半径
 ...
 * @param {} shape
 */
Utils.getShapeAnchorInLinker = function (shape) {
    var anchors = shape.getAnchors();
    var anchorPoints = [];
 ...
 * @return {}
 */
Utils.getEndpointAngle = function (linker, pointType) {
    var point;
    if (pointType == "from") {
        point = linker.from;
    } else {
 ...
 * @param {} point
 */
Utils.getAngle = function (last, point) {
 ...
 * @return {}
 */
Utils.getDarkerColor = function (rgbStr, change) {
    if (!change) {
        change = 13;
    }
 ...
 * @return {}
 */
Utils.getDarkestColor = function (rgbStr) {
 ...
 * 把一个对象的数值属性按缩放比例恢复
 */
Utils.restoreScale = function (obj) {
    var result = {};
    for (var key in obj) {
        result[key] = obj[key]
        if (typeof obj[key] == "number") {
 ...
 * 获取选中图形以外的连接线
 */
Utils.getOutlinkers = function (shapes) {
    var outlinkers = [];
    var outlinkerIds = [];
 ...
 * @return {}
 */
Utils.getFamilyShapes = function (shapes) {
    var familyShapes = [];
    for (var i = 0; i < shapes.length; i++) {
        var shape = shapes[i];
        if (shape.name != "linker") {
 ...
 * @param {} shape
 */
Utils.getChildrenShapes = function (shape) {
    var childrenShapes = [];
 ...
 * @param {} shape2
 */
Utils.isFamilyShape = function (shape1, shape2) {
 ...
 * 获取BPMN吸附的图形（未选中的）
 */
Utils.getAttachedShapes = function (shapes) {
    var ids = [];
    for (var i = 0; i < shapes.length; i++) {
 ...
 */
Utils.rangeChildren = function (shape) {
    var changed = [];
    if (shape.children && shape.children.length > 0) {
 ...
 * @return {}
 */
Utils.getRelativePos = function (pageX, pageY, related) {
