...
Designer.status = "",
    ...
    Designer.initialize = {
        ...
        initShapes: function () {
            ...
            function appendPanelItem(shape, group) {
                ...
                var html = "<div class='panel_box' shapeName='" +
                    shape.name +
                ...
                if (group) {
                    panelBox.append("<div class='group_icon' onmousedown='Designer.op.showPanelGroup(\"" +
                        group +
                        "\", event, this)'></div>")
                }
                var canvas = panelBox.children()[0];
                //绑定鼠标悬浮时，显示大图
                panelBox.bind("mouseenter",
                    function () {
                        ...
                        ctx.translate(-(props.w / 2), -(props.h / 2));
                        Designer.painter.renderShapePath(ctx,
                            shape,
                            false,
                            function () {
                                        ...
                                        for (var i = 0; i < anchorInLinkers.length; i++) {
                                            ...
                                            if (anchorInLinker.type == "line") {
                                                ...
                                                if (anchorInLinker.anchors.length == 1) {
                                                    ...
                                                    var angle = Utils.getPointAngle(createdShape.id,
                                                        anchor.x,
                                                        anchor.y,
                                                        radius);
                                                    linker.to = {
                                                    ...
                                                    //计算新创建的连接线
                                                    angle = Utils.getPointAngle(createdShape.id,
                                                        fromAnchor.x,
                                                        fromAnchor.y,
                                                        radius);
                                                    newLinker.from = {
            ...
            function getCreatingCanvas(name) {
                ...
                if (canvas.length == 0) {
                    ...
                    canvas = $("<canvas id='creating_shape_canvas' width='" +
                        (Designer.config.panelItemWidth) +
                        "' height='" +
                        (Designer.config.panelItemHeight) +
    ...
    Designer.op = {
        ...
        init: function () {
            ...
            container.unbind("mousemove.operate").bind("mousemove.operate",
                function (hoverEvent) {
                            ...
                            function (downE) {
                                ...
                                $(document).bind("mousemove.rotate",
                                    function (moveE) {
                                        ...
                                        //计算旋转角度
                                        var angle = Math.atan(Math.abs(pos.x - scaledCenter.x) /
                                            Math.abs(scaledCenter.y - pos.y));
                                        ...
                                        for (var i = 0; i < selected.length; i++) {
                                            ...
                                            if (shape.name != "linker") {
                                                //旋转形状
                                                shape.props.angle =
                                                    Math.abs((changedAngle + persis.props.angle) % (Math.PI * 2));
                                                var shapeCenter = {
                                                ...
                                                if ((Utils.isSelected(shape.id) && shape.from.id == null) ||
                                                    Utils.isSelected(shape.from.id)) {
                                                    var rotated = Utils.getRotated(center, persis.from, changedAngle);
                                                    shape.from.x = rotated.x;
                                                    shape.from.y = rotated.y;
                                                    if (shape.from.angle != null) {
                                                        shape.from.angle =
                                                            Math.abs((persis.from.angle + changedAngle) %
                                                                (Math.PI * 2));
                                                    }
                                                    fromChanged = true;
                                                }
                                                var toChanged = false;
                                                if ((Utils.isSelected(shape.id) && shape.to.id == null) ||
                                                    Utils.isSelected(shape.to.id)) {
                                                    var rotated = Utils.getRotated(center, persis.to, changedAngle);
                                                    shape.to.x = rotated.x;
                                                    shape.to.y = rotated.y;
                                                    if (shape.to.angle != null) {
                                                        shape.to.angle =
                                                            Math.abs((persis.to.angle + changedAngle) % (Math.PI * 2));
                                                    }
                                                    toChanged = true;
                                                }
        ...
        groupShapeChangable: function () {
            $(".change_shape_icon").bind("mousedown",
                function (e) {
                    ...
                    Designer.op.groupDashboard(groupName,
                        left,
                        top,
                        function (shapeName) {
                            if (targetShape.name != shapeName) {
                                var related = Designer.events.push("shapeChanged",
                                    { shape: targetShape, name: shapeName });
        ...
        shapeMultiSelectable: function () {
            ...
            layout.unbind("mousedown.multiselect").bind("mousedown.multiselect",
                function (downE) {
                    var selector = null;
                    if (!downE.ctrlKey) {
                        Utils.unselect();
                    ...
                    layout.bind("mousemove.multiselect",
                        function (moveE) {
                            if (selector == null) {
        ...
        shapeEditable: function (shape) {
            var canvas = $("#designer_canvas");
            canvas.unbind("dblclick.edit").bind("dblclick.edit",
                function (e) {
                    //计算点击位置在图形的哪个文本区域上
        ...
        shapeLinkable: function (shape, linkPoint) {
            var canvas = $("#designer_canvas");
            var container = $("#canvas_container");
            container.unbind("mousedown.link").bind("mousedown.link",
                function (downE) {
        ...
        linkerEditable: function (linker) {
            var canvas = $("#designer_canvas");
            canvas.unbind("dblclick.edit_linker").bind("dblclick.edit_linker",
                function () {
        ...
        linkerDraggable: function (linker, point) {
            var canvas = $("#designer_canvas");
            var container = $("#canvas_container");
            canvas.bind("mousedown.draglinker",
                function (downE) {
                    Utils.hideLinkerControls();
                    ...
                    container.bind("mousemove.draglinker",
                        function (moveE) {
                            container.css("cursor", "default");
                            var now = Utils.getRelativePos(moveE.pageX, moveE.pageY, canvas);
        ...
        textCreatable: function () {
            var canvas = $("#designer_canvas");
            var container = $("#canvas_container");
            container.unbind("mousedown.create_text").bind("mousedown.create_text",
                function (downE) {
                    var selector = null;
                    if (!downE.ctrlKey) {
                        Utils.unselect();
                    ...
                    var dLocation = Utils.getRelativePos(downE.pageX, downE.pageY, canvas);
                    var location = null;
                    container.bind("mousemove.create_text",
                        function (moveE) {
                            if (selector == null) {
                    ...
                    $(document).unbind("mouseup.create_text").bind("mouseup.create_text",
                        function (upE) {
                            if (location != null && location.width >= 20 && location.height >= 20) {
                                //判断选取范围内的图形
                                var shape = Model.create("standardText",
                                    location.left.restoreScale(),
                                    location.top.restoreScale());
        ...
        canvasDraggable: function () {
            ...
            this.canvasDragTimeout = setTimeout(function () {
                ...
                500);
            container.unbind("mousedown.drag_canvas").bind("mousedown.drag_canvas",
                function (downE) {
                    var beginTop = $("#designer_layout").scrollTop();
                    var beginLeft = $("#designer_layout").scrollLeft();
                    container.bind("mousemove.drag_canvas",
                        function (moveE) {
        ...
        moveShape: function (shapes, offset) {
            ...
            Designer.op.showTip("X: " +
                Math.round(controlPos.left.restoreScale()) +
                "&nbsp;&nbsp;Y: " +
                Math.round(controlPos.top.restoreScale()));
                ...
                if (focus.type == "bounding") {
                    ...
                    if (shape.id == fixedId) {
                        ...
                        for (var ai = 0; ai < anchors.length; ai++) {
                            var an = anchors[ai];
                            var anchorPos = Utils.getRotated(shapeCenter,
        ...
        showLinkPoint: function (point) {
            var canvas = $("<canvas class='link_point_canvas' width=32 height=32></canvas>")
        ...
        brokenLinkerChangable: function (linker, index) {
            ...
            canvas.bind("mousedown.brokenLinker",
                function (downE) {
                    Designer.op.changeState("changing_broken_linker");
                    //初始坐标，要取相对画布的坐标
                    var begin = Utils.getRelativePos(downE.pageX, downE.pageY, canvas);
                    var selectedIds = Utils.getSelectedIds();
                    container.bind("mousemove.brokenLinker",
                        function (moveE) {
                            var now = Utils.getRelativePos(moveE.pageX, moveE.pageY, canvas);
                            //计算和开始时候的偏移量
                            var offset = {
        ...
        snapLine: function (p, ids, isCreate, createdShape) {
                ...
                for (var i = Model.orderList.length - 1; i >= 0; i--) {
                    ...
                    if (shape.name != "linker" && shape.id != boundaryEvent.id) {
                        ...
                        if (result.attach == null &&
                            shapep.angle == 0 &&
                            (shape.groupName == "task" ||
                                shape.groupName == "callActivity" ||
                                shape.groupName == "subProcess")) {
            ...
            if (result.attach == null) {
                ...
                for (var i = Model.orderList.length - 1; i >= 0; i--) {
                    var shapeId = Model.orderList[i].id;
                    var shape = Model.getShapeById(shapeId);
                    if (shape.name == "linker" || ids.indexOf(shapeId) >= 0 || shape.parent) {
                        continue;
                        //排除连接线、已选图形、子图形
                    }
                    var shapep = shape.props;
        ...
        linkDashboard: function (linker) {
            ...
            if ($("#panel_" + category).length != 0) {
                ...
                if (board.length == 0) {
                    //此分类的画板不存在，则初始化
                    board = $("<div id='shape_dashboard_" + category + "' class='shape_dashboard menu'></div>")
                        .appendTo("#designer_canvas");
                     ...
                     */
                    function appendPanelItem(shape, group) {
                        var html = "<div class='dashboard_box' shapeName='" +
                            shape.name +
                            "'><canvas title='" +
                            shape.title +
                ...
                board.find(".link_shape_icon").unbind().bind("mousedown",
                    function (e) {
                        e.stopPropagation();
                        ...
                        Designer.op.groupDashboard(group,
                            left,
                            top,
                            function (name) {
                                linkShape(name);
                ...
                board.children(".dashboard_box").unbind().bind("click",
                    function () {
                        board.hide();
                        $(document).unbind("mousedown.dashboard");
                        var current = $(this);
        ...
        groupDashboard: function (groupName, left, top, onSelected) {
            ...
            if (board.length == 0) {
                //此分类的画板不存在，则初始化
                board = $("<div id='shape_group_dashboard_" + groupName + "' class='group_dashboard menu'></div>")
                    .appendTo("#designer_canvas");
                ...
                for (var i = 0; i < groupShapes.length; i++) {
                    ...
                    if (shape.attribute.visible) {
                        var box = $("<div class='dashboard_box' shapeName='" +
                            name +
                            "'><canvas title='" +
                            shape.title +
                            ...
                            "' height='" +
                            (Designer.config.panelItemHeight) +
                            "'></canvas></div>").appendTo(board);
                        var canvas = box.children("canvas")[0];
                        Designer.painter.drawPanelItem(canvas, shape.name);
                    }
                }
                board.bind("mousedown",
                    function (e) {
                        e.stopPropagation();
                    });
            }
        ...
        showPanelGroup: function (groupName, event, target) {
            ...
            if (board.length == 0) {
                //此分类的画板不存在，则初始化
                board = $("<div id='group_dashboard_" + groupName + "' class='group_dashboard menu'></div>")
                    .appendTo("#designer");
                var groupShapes = SchemaGroup.getGroup(groupName);
                for (var i = 0; i < groupShapes.length; i++) {
                    var name = groupShapes[i];
                    var shape = Schema.shapes[name];
                    if (shape.attribute.visible) {
                        var box = $("<div class='panel_box' shapeName='" +
                            name +
                            "'><canvas title='" +
                            shape.title +
                            "' title_pos='right' width='" +
 ...
 * 方法定义
 */
Designer.addFunction = function (fnName, fnBody) {
    if (Designer[fnName]) {
...
Designer.painter = {
            ...
            if (lineStyle.lineWidth) {
                ...
                if (lineStyle.lineStyle == "dashed") {
                    ...
                    this.setLineDash(ctx,
                        [
                            lineStyle.lineWidth * 6, lineStyle.lineWidth * 2, lineStyle.lineWidth,
    ...
    renderShape: function (shape) {
        ...
        if (shapeBox.length == 0) {
            ...
            shapeBox = $("<div id='" + shape.id + "' class='shape_box'><canvas class='shape_canvas'></canvas></div>")
                .appendTo(superCanvas);
        }
        //得到图形旋转后的矩形边界
    ...
    renderText: function (shape, shapeBox) {
        ...
        for (var i = 0; i < tbs.length; i++) {
            ...
            if (textarea.length == 0) {
                textarea = $("<textarea class='text_canvas' forshape='" + shape.id + "' ind='" + i + "'></textarea>")
    ...
    renderDataAttributes: function (shape, shapeBox) {
        ...
        function renderAttribute(attr, text, icon) {
            ...
            var canvas = $("<canvas id='attr_canvas_" + attr.id + "' class='attr_canvas'></canvas>")
                .appendTo($("#" + shape.id));
            var ctx = canvas[0].getContext("2d");
            var font = "12px ";
            ...
            canvas.attr({
                width: rotated.w.toScale(),
                height: rotated.h.toScale()
            });
            ctx.font = font;
            ...
            if (icon != "") {
                ...
                if (image.length == 0) {
                    image = $("<img class='shape_img' loaded='false' src='" + location + "'/>")
    ...
    renderLinker: function (linker, pointChanged) {
        ...
        if (linkerBox.length == 0) {
            ...
            linkerBox = $("<div id='" +
                linker.id +
                "' class='shape_box linker_box'><canvas class='shape_canvas'></canvas></div>").appendTo(superCanvas);
        }
