 ...
 * @param {} ctx
 */
GradientHelper.createLinearGradient = function (shape, ctx, fillStyle) {
    var p = shape.props;
    //线性渐变
    var begin;
    var end;
 ...
 * @param {} ctx
 */
GradientHelper.createRadialGradient = function (shape, ctx, fillStyle) {
    var p = shape.props;
    var length = p.h;
    if (p.w < p.h) {
        length = p.w;
 ...
 * 获取较淡的颜色
 */
GradientHelper.getLighterColor = function (rgbStr) {
    var change = 60;
 ...
 * @return {}
 */
GradientHelper.getDarkerColor = function (rgbStr) {
    var change = 60;
    var rgb = rgbStr.split(",");
    var r = parseInt(rgb[0]);
    var g = parseInt(rgb[1]);
